...
function hello(d,m,y){
 ...
 function findEtbData(predicate){
				    ...
				    $.ajax({
                       url:"http://" + ip +":8080/box/findRequest",
                       type:"POST",
                ...
                headers:{
                       ...
                       success:function(data){
                         ...
                         $("#table4").append("<tr class='odd grade'>"+
                                       "<td class='center'>"+data.content[i].boxCode+"</td>"+
                                        "<td class='center' onclick='changeStatus(" + JSON.stringify(data.content[i].boxCode) +")' ><a>"+data.content[i].status+"</a></td>"+
										"<td class='center'>"+data.content[i].quantity+"</td>"+
                                        "</tr>")
                                };
...
function nextPage(newPage,totalPage){
	request=request2;
       ...
       function someDay(){             
              ...
              function changeStatus(boxCode){  
                  document.getElementById("boxText2").value = boxCode;
           				  
                    $("#mainBox").hide("slow")
                    $("#changeStatus").show("slow")
                    $("#Button1").show("slow")
 ...
 function changesave(){  
             var predicate;       
				...
				var  request = {
								 ...
								 "linkedPredicate": predicate,
								 "searhRequest": $("#boxText2").val()
								 
                                    }			   
				    $.ajax({
                       url:"http://" + ip +":8080/box/changeStatus",
                       type:"POST",
                       contentType:"application/json",
                headers:{
                                                      token:localStorage.getItem("authorityToken"),
                       ...
                       data:JSON.stringify(request),
                       success:function(data){
						  document.getElementById("boxText2").value = "";  
                              $("#mainBox").show("slow")
                             $("#changeStatus").hide("slow")
                             $("#Button1").hide("slow")
							  },
